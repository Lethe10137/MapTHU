// from https://uoj.ac/submission/657571
...
namespace MWM{
  ...
  vector<int> solve_mwm(int N, int M, vector<Edge> edges) {
    ...
    for (int i = 0; i < M; ++i) { 
      ou[edges[i].from + 1] += 1; ov[edges[i].to + 1] += 1;
    }
...
namespace Dijkstra{
  ...
  int edge_cnt = 1;// edge 0 and 1 is not used

  int first_edge[N];
  int degree_cnt[N];

  int get_weight(int s, int t){
      double square = (x[s] - x[t]) * (x[s] - x[t]) + (y[s] - y[t]) * (y[s] - y[t]); 
      square = sqrt(square);
      return (int)square;
  }
  ...
  void add_edge(int s, int t){
      edge_cnt ++;
      int weight = get_weight(s,t);

      edges[edge_cnt] = {t,weight,first_edge[s], edge_cnt + 1,true};
      first_edge[s] = edge_cnt;
      edge_cnt ++;
      edges[edge_cnt] = {s,weight,first_edge[t], edge_cnt -1 ,true};
      first_edge[t] = edge_cnt;
      degree_cnt[s] += 1;
      degree_cnt[t] += 1;
  }
  ...
  void dijkstra(int s, int t){
      ...
      while( !q.empty() )
      {
          ...
          if( vis[x] )
              continue;
          vis[x] = 1;
          for( int i = first_edge[x]; i; i = edges[i].next_edge )
          {
              int y = edges[i].to;
  ...
  void solve(){
    ...
    for(int i = 0; i < n; i++){
        ...
        for (int j = 0; j < edges; j++){
            ...
            add_edge(id, neighbor);// id > neighbour, thus the coord for both have been updated
        }
    }

    int single = 0;
    for(int i = 1; i <= n; i++){
        if(degree_cnt[i] % 2 == 1)single++;
    }

    std::vector<MWM::Edge> edges_dj;

    for(int i = 1; i <= n; i++){
      if(degree_cnt[i] % 2 == 0)continue;
      dijkstra(i, -1);
      for(int j = i + 1; j <= n; j++){
          if(degree_cnt[j] % 2 == 0)continue;
          edges_dj.push_back({i,j,1000000 - dis[j]});
      }
    }

    std::vector<int> link_to_delete = MWM::solve_mwm(n, edges_dj.size(), edges_dj);

    std::vector<std::pair<int, int>> pairs_to_kill = {};
    int i = 0;
    for(auto a: link_to_delete){
      if(i != 0 && a != 0 && i < a){        
        dijkstra(i,a);

        do{
          pairs_to_kill.push_back({a, last[a]});
          pairs_to_kill.push_back({last[a], a});
          a = last[a];
        }while(a != i);
      }
      i += 1;
    ...
    std::sort(pairs_to_kill.begin(), pairs_to_kill.end(), [](const std::pair<int, int>& a, const std::pair<int, int>& b) {
        if (a.first != b.first)
    ...
    auto p = pairs_to_kill.begin();

    for(int s = 1; s <= n; s++){
      if(s < p->first)continue;
      for( int t = first_edge[s]; t; t = edges[t].next_edge ){
        if(s < p->first)break;
        if(edges[t].to > p->second)continue;
        assert(s == p->first && edges[t].to == p->second);
        assert(edges[t].hold);
        edges[t].hold = false;
        p++;
      }
    }
    assert(p == pairs_to_kill.end());
  ...
  void dfs(int s){
    for(int t = first_edge[s]; t; t = edges[t].next_edge ){
      if(edges[t].hold){
        edges[t].hold = false;
        edges[edges[t].rev].hold = false;
        total += edges[t].weight;
        dfs(edges[t].to);
      }
    }
    printf("%d->", s);
...
int main(){
  Dijkstra::solve();
  Dijkstra::dfs(1);
  fprintf(stderr, "%lf\n", (double)Dijkstra::total * 210.0 / 686);
}